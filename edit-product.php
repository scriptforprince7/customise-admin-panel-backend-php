...
if(isset($_POST['update_product']))
{
	...
	{
	$prod_img='img_'.rand(0,1000).'_'.$_FILES['prod_img']['name'];
	}
	else
	{
	$prod_img=$_POST['prod_img_hidden'];
	}


	if(empty($prod_img_alt_tag))
	{
		$err[]='Product Image Alt Tag is required';
	}
	...
	if(count($err)==0)
	{
		 ...
		 $query=mysqli_query($conn,"update tbl_product
    	                                             SET prod_name='$prod_name',
    	                                                 prod_img='$prod_img',
    	                                                 prod_img_alt_tag='$prod_img_alt_tag',
    	                                                 prod_descp='$prod_descp',
    	                                                 prod_slug='$prod_slug',
    	                                                 is_active=$is_active where prod_id=$edit_id");
    ...
    if($query)
    {
    		...
    		if(!empty($_FILES['prod_img']['name']))
	{
    	move_uploaded_file($_FILES['prod_img']['tmp_name'], 'images/product/'.$prod_img.'');
    	unlink('images/product/'.$_POST['prod_img_hidden']);
    }
...
$details=mysqli_fetch_array(mysqli_query($conn,"select * from tbl_product where prod_id=$edit_id"));
...
<!--begin::Button-->
<a href="<?=BASE_URL?>manage-product.php" class="btn btn-primary font-weight-bolder">
...
<input type="text" name="prod_name" value="<?=$details['prod_name']?>" placeholder="Name" class="form-control">
</div>
...
<?php if(!empty($details['prod_img'])){?>
<img src="<?=BASE_URL_IMG?>product/<?=$details['prod_img']?>" width='50' class="mt-3">
<?php }?>
<input type="hidden" name="prod_img_hidden" value="<?=$details['prod_img']?>">
</div>
...
<span class="text-danger">*</span></label>
<input type="text" name="prod_img_alt_tag" value="<?=$details['prod_img_alt_tag']?>" placeholder="Image Alt Tag" class="form-control">
</div>

<div class="form-group">
<label for="exampleInputPassword1">Description
<span class="text-danger">*</span></label>
<textarea name="prod_descp" placeholder="Description" class="form-control" id="kt-tinymce-4"><?=$details['prod_descp']?></textarea>
</div>
...
<span class="text-danger">*</span></label>
<input type="text" name="prod_slug" value="<?=$details['prod_slug']?>" placeholder="Slug" class="form-control">
</div>
...
<button type="submit" name="update_product" class="btn btn-primary mr-2">Update</button>
